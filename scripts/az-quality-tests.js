...
(async () => {
  ...
  ok('greenlight refuses without ALLOW_GREENLIGHT', fs.readFileSync(path.join(ROOT, 'scripts/greenlight_apply.js'), 'utf8').includes("ALLOW_GREENLIGHT !== '1'"));
  const flagshipWriter = fs.readFileSync(path.join(ROOT, 'scripts/write_flagship_guides.js'), 'utf8');
  ok('write_flagship_guides refuses without ALLOW', flagshipWriter.includes("ALLOW_WRITE_FLAGSHIP_GUIDES !== '1'"));
  ok('write_flagship_guides canonical strips .html', flagshipWriter.includes('prettySlug') && !flagshipWriter.includes('guides/${g.slug}'));
  const redirects = fs.readFileSync(path.join(ROOT, '_redirects'), 'utf8');
  ok('legacy community 301s to pretty /contact', redirects.includes('/community.html /contact 301') && !redirects.includes('/community.html /contact.html'));
  ok('legacy resize-image-online 301s to pretty guide', redirects.includes('/guides/resize-image-online.html /guides/how-to-resize-image-online 301'));
  ok('legacy first-post 301s to pretty blog URL', redirects.includes('/blog/first-post.html /blog/image-compression-best-practices 301'));
  ok('ai-generate-blog refuses without ALLOW_AI_BLOG', fs.readFileSync(path.join(ROOT, 'scripts/ai-generate-blog-post.js'), 'utf8').includes("ALLOW_AI_BLOG !== '1'"));
  ok('run1-fixes refuses without ALLOW_RUN1_FIXES', fs.readFileSync(path.join(ROOT, 'scripts/run1-fixes.js'), 'utf8').includes("ALLOW_RUN1_FIXES !== '1'"));
