...
const descriptions = {
  ...
  'flip-image': 'Free online flip image tool for product photos and design. Mirror horizontally or vertically in your browser. No signup, instant download.',
  ...
  'image-saturation': 'Free online saturation tool for product photos and social posts. Boost or mute colors in your browser. No signup, live preview sliders.',
  ...
  'image-brightness': 'Free online brightness adjuster for product photos and listings. Fix dark or washed-out images in your browser. No signup, live preview.',
};
...
for (const slug of fs.readdirSync(TOOLS)) {
  ...
  if (!next) {
    console.warn('No description for', slug);
    continue;
  }
  ...
  updated++;
  console.log(`${slug}: ${current.length} → ${next.length} chars`);
}

console.log(`\nUpdated ${updated}, skipped ${skipped} (already >= 100 chars)`);
